...
def make_dtopo(makeplots=False):
    """
    Create dtopo data file for deformation of the seafloor due to an earthquake.
    Uses the Okada model with fault parameters and mesh specified below.
    """
    from clawpack.geoclaw import dtopotools
    import numpy

    dtopo_fname = os.path.join(scratch_dir, "dtopo_manila_trench.tt3")

    # Specify subfault parameters for the Manila Trench:
    manila_trench_subfault = dtopotools.SubFault()
    manila_trench_subfault.strike = 354.
    manila_trench_subfault.length = 160.e3
    manila_trench_subfault.width = 20.e3
    manila_trench_subfault.depth = 40.e3
    manila_trench_subfault.slip = 90.
    manila_trench_subfault.rake = 10.
    manila_trench_subfault.dip = 20.
    manila_trench_subfault.longitude = 120.5
    manila_trench_subfault.latitude = 20.2
    manila_trench_subfault.coordinate_specification = "top center"

    fault = dtopotools.Fault()
    fault.subfaults = [manila_trench_subfault]
    ...
    if os.path.exists(dtopo_fname):
        ...
        x = numpy.linspace(100, 121, 100)
        y = numpy.linspace(5, 21, 100)
        times = [1.]
